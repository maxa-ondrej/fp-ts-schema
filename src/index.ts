...
export interface Decoder<D> {
	readonly decode: (
		data: unknown,
	) => Either<DecoderError, ReturnType<Decode<D>["forceDecode"]>>;
...
export type Output<T extends Decoder<any>> = T extends Decoder<infer D>
	? D
	: never;
...
export const createDecoder = <D>(decoder: Decode<D>): Decoder<D> => ({
	decode: (data) => {
		try {
			...
			return left(
				e instanceof DecoderError ? e : new DecoderError("Invalid data"),
			);
		}
	},
	is: (data): data is D => {
 ...
 */
export const nullable = <D>(decoder: Decoder<D>): Decoder<Option<D>> =>
	oneOf(decoder, null_).andThen(fromNullable);
...
export const literal = <D extends string | number | boolean>(
	literal: D,
): Decoder<D> =>
	createDecoder({
		forceDecode: (data) => {
			...
			if (data !== literal) {
				throw new DecoderError(
					`Data does not match the literal. Expected: '${
						literal as string
					}', actual value: '${show(data)}'`,
				);
			}
			return data as D;
		},
...
type AllOf<D extends Decoder<MapObject<any>>[]> = D extends [
	infer A,
	...infer B,
]
...
export const allOf = <D extends Decoder<MapObject<any>>[]>(...decoders: D) =>
	createDecoder({
		forceDecode: (data) =>
			decoders.reduce(
				(acc, decoder) => deepMerge(acc, forceDecode(decoder, data)),
				{},
			) as AllOf<D>,
	});

export const literalUnion = <
	D extends ReadonlyArray<string | number | boolean>,
>(
	...decoders: D
): Decoder<D[number]> => oneOf(...decoders.map(literal));

export const regex = (regex: RegExp): Decoder<string> =>
	string.andThen((data) => {
		if (!regex.test(data))
			throw new DecoderError(
				`Data '${data}' does not satisfy the regex '${regex.toString()}'`,
			);
...
export const array = <D>(decoder: Decoder<D>): Decoder<D[]> =>
	createDecoder({
		forceDecode: (data) => {
			checkDefined(data);
			checkArrayType(data);
			return data.map((x: unknown, i) =>
				forceDecodeWithPath(decoder, x, i.toString()),
			);
		},
	});
...
function checkDictType(data: unknown): asserts data is Record<string, unknown> {
	if (typeof data !== "object" || data === null || Array.isArray(data)) {
		...
		Object.keys(data).some(($) => typeof $ !== "string") ||
		Object.getOwnPropertySymbols(data).length > 0
	) {
		throw new DecoderError(
			`Not all keys in this object are strings: ${show(data)}`,
		);
	}
}
		...
		forceDecode: (data) => {
			...
			return Object.fromEntries(
				Object.entries(data).map(([key, value]) => [
					key,
					forceDecodeWithPath(decoder, value, key),
				]),
			);
		},
	});

export const keyValuePairs = <D>(
	decoder: Decoder<D>,
): Decoder<Array<[string, D]>> =>
	createDecoder({
...
type ObjectTypeRequired<D extends DecoderRecord> = {
	[K in keyof D]: Output<D[K]>;
};
type ObjectTypeOptional<D extends DecoderRecord> = {
	[K in keyof D]: Option<Output<D[K]>>;
};

const required = <D extends DecoderRecord>(
	struct: D,
): Decoder<ObjectTypeRequired<D>> =>
	createDecoder({
		forceDecode: (data) => {
			checkDictType(data);
...
export const structuredObject = <
	D extends DecoderRecord,
	E extends DecoderRecord,
>(struct: {
	required?: D;
	optional?: E;
}): Decoder<ObjectTypeRequired<D> & ObjectTypeOptional<E>> =>
	createDecoder({
		forceDecode: (data) => {
			checkDefined(data);
...
export const recursive = <D>(decoder: () => Decoder<D>): Decoder<D> =>
	createDecoder({
		forceDecode: (data) => {
			return forceDecode(decoder(), data);
		},
