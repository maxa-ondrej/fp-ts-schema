...
const primitiveTest = <T>(
	...
	test(`${name} succeeds when given the correct type`, () => {
		for (const $ of values.success) {
			shouldBe(decoder, $, $);
		}
	});
	test(`${name} fails when given a wrong type`, () => {
		for (const $ of [...values.failure, {}, []]) {
			shouldFail(decoder, $);
		}
	});
...
test("D.badDecoder inside array fails with the given error", () => {
	const badDecoder: D.Decoder<never> = {
		decode: () =>
			either.left(new Error("test")) as Either<D.DecoderError, never>,
...
test("D.succeed succeeds with the given value", () => {
	...
	shouldBe(D.oneOf(D.string, D.succeed("test")), Number.NaN, "test");
});

// Never
test("D.never fails with the given value", () => {
	shouldFail(D.never, null);
	shouldFail(D.oneOf(D.string, D.never), Number.NaN);
...
test("D.oneOf succeeds when given one of the permitted types", () => {
	...
	shouldBe(
		D.oneOf(D.number, D.nullable(D.string)),
		"test",
		option.some("test"),
	);
...
test("D.allOf succeeds when given one all needed", () => {
	shouldBe(
		D.allOf(D.object({ a: D.number }), D.object({ b: D.string })),
		{ a: 5, b: "test" },
		{ a: 5, b: "test" },
	);
	shouldBe(
		D.allOf(
			D.object({ a: D.object({ a: D.number }) }),
			D.object({ b: D.string }),
		),
		{ a: { a: 5 }, b: "test" },
		{ a: { a: 5 }, b: "test" },
	...
	shouldBe(
		D.allOf(
			D.object({ a: D.object({ a: D.number, c: D.boolean }) }),
			D.object({ a: D.object({ b: D.string, c: D.boolean }) }),
		),
		{ a: { a: 5, b: "test", c: true } },
		{ a: { a: 5, b: "test", c: true } },
	);
...
test("D.allOf fails when at least one is missing", () => {
	shouldFail(D.allOf(D.object({ a: D.number }), D.object({ b: D.string })), {
		a: 5,
	});
	shouldFail(D.allOf(D.object({ a: D.number }), D.object({ b: D.string })), {
		b: "test",
...
test("D.keyValuePairs succeeds when given a dict", () => {
	shouldBe(D.keyValuePairs(D.number), { a: 1, b: 2 }, [
		["a", 1],
		["b", 2],
	]);
...
test("D.object succeeds when it has all required fields", () => {
	shouldBe(
		D.object({ foo: D.string, bar: D.number }),
		{ foo: "test", bar: 5 },
		{ foo: "test", bar: 5 },
	);
});
...
test("D.recursive succeeds when used correctly", () => {
	...
	const userDecoder: D.Decoder<User> = D.tuple(
		D.string,
		D.string,
		D.array(D.recursive(() => userDecoder)),
	);
...
test("DecodeError path of a recursive type is correct", () => {
	...
	if (either.isLeft(result)) {
		expect(result.left.message).toStrictEqual(
			"subcategories.0.subcategories.1.name: This is not a string: 1",
		);
	} else {
		fail("Expected error");
	}
